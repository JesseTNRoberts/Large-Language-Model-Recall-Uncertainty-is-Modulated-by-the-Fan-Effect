...
def generate_prompts(members, random_member_typical, random_member_atypical, loc):
    ...
    list = (random_member_atypical + random_member_typical + ['hoodie'])
    random.shuffle(list)
    list = ", ".join(list)
    ...
    for member in members:
        preamble = f"Following is a list that contains a number of clothes. After the list, a clothing item will be judged as either present or absent in the list. If the list contains the clothing item, answer with present. If the list does not contain the clothing item, answer with absent. The list of clothes is: {list}. According to the list, hoodie is present. According to the list, leggings are absent. According to the list, {member} is "
...
def call_function():
    loc = 'C:/Users/phamt2/evaluating_fan_effects_in_large_language_models/Experiments/Typicality/data/clothing/clothing.txt'
