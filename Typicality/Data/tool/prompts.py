...
def generate_prompts(members, random_member_typical, random_member_atypical, loc):
    ...
    list = (random_member_atypical + random_member_typical + ['pliers'])
    random.shuffle(list)
    list = ", ".join(list)
    ...
    for member in members:
        preamble = f"Following is a list that contains a number of tools. After the list, a tool will be judged as either present or absent in the list. If the list contains the tool, answer with present. If the list does not contain the tool, answer with absent. The list of tools is: {list}. According to the list, pliers are present. According to the list, tape measure is absent. According to the list, {member} is "
...
def call_function():
    loc = 'C:/Users/phamt2/evaluating_fan_effects_in_large_language_models/Experiments/Typicality/data/tool/tool.txt'
