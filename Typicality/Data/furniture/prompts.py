...
def generate_prompts(members, random_member_typical, random_member_atypical, loc):
    ...
    list = (random_member_atypical + random_member_typical + ['bookcase'])
    random.shuffle(list)
    list = ", ".join(list)
    ...
    for member in members:
        preamble = f"Following is a list that contains a number of furniture. After the list, a furniture will be judged as either present or absent in the list. If the list contains the furniture, answer with present. If the list does not contain the furniture, answer with absent. The list of furniture is: {list}. According to the list, bookcase is present. According to the list, wardrobe is absent. According to the list, {member} is "
...
def call_function():
    loc = 'C:/Users/phamt2/evaluating_fan_effects_in_large_language_models/Experiments/Typicality/data/furniture/furniture.txt'
