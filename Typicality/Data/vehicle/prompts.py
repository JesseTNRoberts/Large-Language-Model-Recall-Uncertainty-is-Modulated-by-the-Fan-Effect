...
def generate_prompts(members, random_member_typical, random_member_atypical, loc):
    ...
    list = (random_member_atypical + random_member_typical + ['minivan'])
    random.shuffle(list)
    list = ", ".join(list)
    ...
    for member in members:
        preamble = f"Following is a list that contains a number of vehicles. After the list, a vehicle will be judged as either present or absent in the list. If the list contains the vehicle, answer with present. If the list does not contain the vehicle, answer with absent. The list of vehicles is: {list}. According to the list, minivan is present. According to the list, SUV is absent. According to the list, {member} is "
...
def call_function():
    loc = 'C:/Users/phamt2/evaluating_fan_effects_in_large_language_models/Experiments/Typicality/data/vehicle/vehicle.txt'
