...
def generate_prompts(members, random_member_typical, random_member_atypical, loc):
    ...
    list = (random_member_atypical + random_member_typical + ['crossbow'])
    random.shuffle(list)
    list = ", ".join(list)
    ...
    for member in members:
        preamble = f"Following is a list that contains a number of weapons. After the list, a weapon will be judged as either present or absent in the list. If the list contains the weapon, answer with present. If the list does not contain the weapon, answer with absent. The list of weapons is: {list}. According to the list, crossbow is present. According to the list, machete is absent. According to the list, {member} is "
...
def call_function():
    loc = 'C:/Users/phamt2/evaluating_fan_effects_in_large_language_models/Experiments/Typicality/data/weapon/weapon.txt'
